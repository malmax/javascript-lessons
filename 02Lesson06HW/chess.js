                ...
                Object.keys(obj).forEach(function(colorName) {
                    Object.keys(obj[colorName]).forEach(function(unitTypeName) {
                        ...
                        unitValues.startLocation.forEach(function(location) {
                            ...
                            unitRender.unit.addEventListener('mousedown',fun.bind(unitRender));
                            unitRender.unit.style.zIndex = 100;
                            //делаем draggable
                            $(unitRender.unit).draggable({ revert: "invalid" });
                        });
                    });
                });
            }
...
function GameUnit(elemType, color, startLocation, unitClass, unitId) {
    ...
    this.makeMoveEvents = function(parentTag, e) {
        //получаем TD куда может походить текущая фигура
        var tds = getRelativeTD(parentTag, e);
        if(tds) {           
            // отмечаем клетки в которые может "походить" фигура
            tds.forEach(function(obj) {
                // закрашиваем TD в зеленый цвет
                obj.dest.classList.add('can-move-there');
                // добавляем droppable в эти TD
                $(obj.dest).droppable({
                    drop: function( event, ui ) {
                        //перемещаем Node-фигуру в дроп-тд 
                        moveTo(event.target);
                        //отменяем изменения стиля у юнита от draggable
                        that.unit.style = "";
                        // удаляем все droppable чтобы туда нельзя было походить снова
